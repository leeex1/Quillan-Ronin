...
def format_math_for_reportlab(m: str) -> str:
    ...
    m = m.replace(r"\epsilon", "ε")
    m = m.replace(r"\xi", "ξ")
    m = m.replace(r"\nabla", "∇")
    m = m.replace(r"\in", "∈")
    m = m.replace(r"\sim", "~")
    m = m.replace(r"\Phi", "Φ")
    m = m.replace(r"\Theta", "Θ")
    m = m.replace(r"\pi", "π")
    m = m.replace(r"\cdot", "·")
    m = m.replace(r"\times", "×")
    m = m.replace(r"\leq", "≤")
    ...
    m = m.replace(r"\geq", "≥")
    m = m.replace(r"\ge", "≥")
    m = m.replace(r"\approx", "≈")
    m = m.replace(r"\mathbb{R}", "ℝ")
    m = m.replace(r"\mathcal{H}", "H")
    m = m.replace(r"\mathcal{L}", "L")
    m = m.replace(r"\exp", "exp")
